...
export class UserAcknowledgeGuard implements CanActivate {
  ...
  private openedDialogRef: MatDialogRef<UserAchnowledgeDialogComponentComponent> | null = null;
      ...
      switchMap( (aAck: boolean) => {
        if (aAck) {
          ...
          if (this.openedDialogRef == null) {
            ...
            this.openedDialogRef = this.matDialog.open(UserAchnowledgeDialogComponentComponent,
              {
                disableClose: true
              }
            );
